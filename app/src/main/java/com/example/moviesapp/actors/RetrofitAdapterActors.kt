package com.example.moviesapp.actors
...
class RetrofitAdapterActors(private var movies : MutableList<Movie>, private val onMovieClick : (movie : Movie) -> Unit) : RecyclerView.Adapter<RetrofitAdapterActors.ViewHolder>() {
    ...
    class ViewHolder(item : View) : RecyclerView.ViewHolder(item){
        ...
        fun Bind(movie: Movie){
            Glide.with(itemView)
                .load("https://image.tmdb.org/t/p/w342${movie.profilePath}")
                .transform(FitCenter())
                .into(image)
        }
    }
